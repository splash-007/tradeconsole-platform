    ...
    <div className="py-4 space-y-4 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Pending', value: pendingCount, color: '#3b82f6', icon: Clock },
        ].map(({ label, value, color, icon: Icon }) => (
          <div key={label} className="rounded-xl border p-3.5 flex items-center gap-3" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)' }}>
      ...
      {/* Filters */}
      <div className="rounded-xl border p-3 flex flex-wrap gap-2 items-center" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)' }}>
        {/* Search */}
              ...
              return (
                <div key={tx.id} className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] gap-3 px-4 py-3.5 hover:bg-white/[0.02] transition-colors">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: typeConf.bg }}>
                  ...
                  <div className="flex items-center md:block">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-md font-medium" style={{ backgroundColor: typeConf.bg, color: typeConf.color }}>
                      {typeConf.label}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center md:block">
                    ...
                    <span className="text-xs font-semibold" style={{ color: isPositive ? '#22c55e' : tx.type === 'fee' ? '#6b7280' : '#ef4444' }}>
                      {isPositive ? '+' : ''}{tx.amount >= 0 ? '' : ''}{tx.amount.toLocaleString()} {tx.currency}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center md:block">
                    ...
                    <span className="text-xs font-medium" style={{ color: 'var(--foreground)' }}>${tx.balance.toLocaleString()}</span>
                  </div>
